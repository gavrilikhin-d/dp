...
use dashmap::DashMap;
use dp_language_server::chumsky::{parse, type_inference, Func, ImCompleteSemanticToken};
use dp_language_server::completion::completion;
use dp_language_server::jump_definition::get_definition;
use dp_language_server::reference::get_reference;
use dp_language_server::semantic_token::{semantic_token_from_ast, LEGEND_TYPE};
use ropey::Rope;
        ...
        let inlay_hint_list = hashmap
            ...
            .map(|(k, v)| {
                (
                    ...
                    k.end,
                    match v {
                        dp_language_server::chumsky::Value::Null => "null".to_string(),
                        dp_language_server::chumsky::Value::Bool(_) => "bool".to_string(),
                        dp_language_server::chumsky::Value::Num(_) => "number".to_string(),
                        dp_language_server::chumsky::Value::Str(_) => "string".to_string(),
                        dp_language_server::chumsky::Value::List(_) => "[]".to_string(),
                        dp_language_server::chumsky::Value::Func(_) => v.to_string(),
                    },
                )
            })
            .filter_map(|item| {
    ...
    async fn completion(&self, params: CompletionParams) -> Result<Option<CompletionResponse>> {
        ...
        let completions = || -> Option<Vec<CompletionItem>> {
            ...
            for (_, item) in completions {
                match item {
                    dp_language_server::completion::ImCompleteCompletionItem::Variable(var) => {
                    ...
                    dp_language_server::completion::ImCompleteCompletionItem::Function(
                        name,
                        args,
                    ) => {
                        ret.push(CompletionItem {
